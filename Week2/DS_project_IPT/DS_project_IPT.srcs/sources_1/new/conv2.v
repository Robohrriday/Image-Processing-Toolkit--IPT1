...
module conv2(input clk,input rst_2,input infer_2,input [14:0]addr_2, output [7:0]out_2, output reg conv_done_2,
output ena_conv2_1,
output ena_conv2_0,
output wea_conv2_1,
output wea_conv2_0,
output [14:0]addr_conv2_1,
output [13:0]addr_conv2_0,
output [7:0]din_conv2_1,
output [7:0]din_conv2_0,
input [7:0]dout_conv2_1,
input [7:0]dout_conv2_0);
...
// BRAM signals
assign ena_conv2_1 = 1;
assign ena_conv2_0 = 1;
assign wea_conv2_1 = 0;
assign wea_conv2_0 = wea2;
assign addr_conv2_1 = addr1;
assign addr_conv2_0 = w_addr;
assign din_conv2_1 = 8'd0;
assign din_conv2_0 = conv;
assign out1 = dout_conv2_1;
assign out = dout_conv2_0;

// Reading filter values from text file. Filter file format given. Change path accordingly.
initial begin
    $readmemb("F:/D/IITGN/ES_204_Digital_Systems/DS_project_IPT/DS_project_IPT.srcs/sources_1/sharp_filter.txt", filter);
end
    ...
    if (slow_clk == 5) begin    
        ...
        case (state)
            ...
            2: begin                                    // State which stores the convolved pixel value in BRAM
                ...
                wea2 <= 1;
                if (sum > 255) conv <= 255;     // ***** Implementation specific to laplacian filter. Change according to use.
                else if (sum < 0) conv <= 0;
                else conv <= sum;
                conv_done_2 <= 0;
            end
            3:begin                                     // Sink state or state for inferencing
                $display("State = 3");
                if (infer_2 & ~rst_2) begin
                    $display("              (3.1a) infer = %d", infer_2);
                    $display("              (3.1b) rst = %d\n", rst_2);
                    wea2 <= 0;
                    w_addr <= addr_2 + 1;
                    state <= 3;
